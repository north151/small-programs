...
int stopspan1=10,stopspan2=10;  //降低旋转灵敏度
		...
		if (m.x>width*0.3 && m.x<width*0.3+50*3 && m.y>high*0.2 && m.y<high*0.2+50  ){   //新游戏
			...
			if(m.uMsg==WM_LBUTTONDOWN){ //如果左键按下
				...
				if(m.x>width*0.2 && m.x<width*0.2+50 && m.y>high*0.5 && m.y<high*0.5+50){
					...
					p1_y=rand()%400;
					p1_angle=rand()%8-4;
					do{ p2_x=rand()%200;}while(p2_x == p1_x);  //干扰，防止两个玩家位置重合
					p2_y=rand()%400;
					p2_angle=rand()%8-4;
					...
					p1_y=rand()%800;
					p1_angle=rand()%8-4;
					do{ p2_x=rand()%400;}while(p2_x == p1_x);  //干扰，防止两个玩家位置重合
					p2_y=rand()%800;
					p2_angle=rand()%8-4;
					...
					p1_y=rand()%550;
					p1_angle=rand()%8-4;
					do{ p2_x=rand()%600;}while(p2_x == p1_x);  //干扰，防止两个玩家位置重合
					p2_y=rand()%550;
					p2_angle=rand()%8-4;
...
void updatewithinput()  //与(游戏中)输入有关的更新
{
  //玩家1
		...
		if ( GetAsyncKeyState(0x20) & 0x8000 ){  //按下空格 玩家1发射子弹	
			int j;
			//检索每个子弹
			for(j=0;j<30;j++){ 
   ...
   /////玩家2
		...
		if ( GetAsyncKeyState(0x0d) & 0x8000 ){  //按下回车 玩家2发射子弹		
			int j;
			//检索每个子弹
			for(j=0;j<30;j++){ 
